...
![创建群组](_static/images/create-group.png)

## 群组管理

点击群组列表中右侧操作栏，群主可以进行对群组进行 编辑、解散、邀请入群操作。用户群组的身份不同，操作权限不同。

![群组管理](_static/images/manage-group.png)
...
![成员管理](_static/images/manager-member.png)

## 群组权限管理

群组云盘面向成员共有三种权限可以配置，**管理**权限、**编辑**权限和**只读**权限，可以点击权限设置按照人员选择配置即可，三类权限区别见下表。
